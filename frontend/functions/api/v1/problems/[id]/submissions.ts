...
function readProblemId(req: Request, params: Record<string, string> | undefined) {
  const p = params?.id
  if (p) return p
  const m = new URL(req.url).pathname.match(/\/api\/v1\/problems\/([^/]+)\/submissions\/?$/)
...
export const onRequestPost: PagesFunction = async ({ request, env, params }) => {
  ...
  if (!problemId) return httpError('INVALID_ARGUMENT', 'problemId required', 400)
  ...
  try {
    ...
    ).bind(submissionId, userId, problemId, code, 'queued', now).run()
  } catch (e) {
  ...
  try {
    await env.QUEUE_SUBMISSIONS.send({ submissionId, problemId, userId, createdAt: now })
  } catch (e) {
  ...
  return httpJson(
    { ok: true, data: { submissionId, userId, problemId, status: 'queued', createdAt: now } },
    201
  )
}
