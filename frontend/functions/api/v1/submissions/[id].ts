...
export const onRequestGet: PagesFunction = async ({ request, env, params }) => {
  ...
  try {
    ...
    ).bind(submissionId).first<{ submissionId: string; userId: string; problemId: string; status: string; createdAt: number }>()
    if (!row) return httpError('NOT_FOUND', 'submission not found', 404)
    if (row.userId !== uid) return httpError('PERMISSION_DENIED', 'not owner', 403)
