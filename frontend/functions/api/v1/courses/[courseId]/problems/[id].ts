...
function readIds(req: Request, params: Record<string, string> | undefined) {
  ...
  const m = new URL(req.url).pathname.match(/\/api\/v1\/courses\/([^/]+)\/problems\/([^/]+)$/)
  return m ? { courseId: decodeURIComponent(m[1]), id: decodeURIComponent(m[2]) } : { courseId: '', id: '' }
}

export const onRequestGet: PagesFunction = async ({ request, env, params }) => {
  const { courseId, id } = readIds(request, params as any)
  if (!courseId || !id) return httpError('INVALID_ARGUMENT', 'courseId and id required', 400)
  try {
    const row = await env.DB
      ...
      .bind(courseId, id)
      .first<{ problemId: string; courseId: string; title: string; description: string | null }>()
