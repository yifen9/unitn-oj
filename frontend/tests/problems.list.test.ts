...
import * as mod from '../functions/api/v1/courses/[courseId]/problems/index'
...
describe('GET /api/v1/courses/{courseId}/problems', () => {
  ...
  it('200 returns empty list when no mapping', async () => {
    ...
    state.allResults = []
    const ctx = makeCtx({ url: 'http://x/api/v1/courses/UNITN_CP1/problems', env: { ...DEV_ENV, DB: db } })
    const res = await (mod as any).onRequestGet(ctx)
  ...
  it('500 INTERNAL when DB throws in prod', async () => {
    ...
    const ctx = makeCtx({ url: 'http://x/api/v1/courses/UNITN_CP1/problems', env: { ...PROD_ENV, DB: db } })
    const res = await (mod as any).onRequestGet(ctx)
