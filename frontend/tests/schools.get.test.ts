...
import * as mod from '../functions/api/v1/schools/[id]'
...
describe('GET /api/v1/schools/{id}', () => {
  ...
  it('404 when not found', async () => {
    const { db } = makeD1Mock()
    const ctx = makeCtx({ url: 'http://x/api/v1/schools/none', env: { ...DEV_ENV, DB: db } })
    const res = await (mod as any).onRequestGet(ctx)
  ...
  it('200 when found', async () => {
    ...
    ;(db as any).prepare = (sql: string) => {
      const s = orig(sql)
      if (/FROM\s+schools\s+WHERE/i.test(sql)) s.first = async () => state.firstResult
      return s
    }
    const ctx = makeCtx({ url: 'http://x/api/v1/schools/ror:unitn', env: { ...DEV_ENV, DB: db } })
    const res = await (mod as any).onRequestGet(ctx)
