...
describe('GET /api/v1/submissions/{id}', () => {
  it('400 when id missing', async () => {
    const { db } = makeD1Mock()
    const ctx = makeCtx({ url: 'http://x/api/v1/submissions/', env: { ...DEV_ENV, DB: db } })
  ...
  it('401 when no sid', async () => {
    const { db } = makeD1Mock()
    const ctx = makeCtx({ url: 'http://x/api/v1/submissions/s1', env: { ...DEV_ENV, DB: db } })
    const res = await (mod as any).onRequestGet(ctx)
  ...
  it('403 when not owner', async () => {
    ...
    state.firstResult = { submissionId: 's1', userId: 'u_owner', problemId: 'p1', status: 'queued', createdAt: 1 }
  ...
  it('200 when owner', async () => {
    ...
    state.firstResult = { submissionId: 's1', userId: uid, problemId: 'p1', status: 'queued', createdAt: 1 }
    const orig = db.prepare.bind(db)
    ;(db as any).prepare = (sql: string) => {
      const s = orig(sql)
      if (/FROM\s+submissions\s+WHERE/i.test(sql)) s.first = async () => state.firstResult
