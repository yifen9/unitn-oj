...
import * as mod from '../functions/api/v1/users/me/submissions'
...
describe('GET /api/v1/users/me/submissions', () => {
  it('401 when no sid', async () => {
    const { db } = makeD1Mock()
    const ctx = makeCtx({ url: 'http://x/api/v1/users/me/submissions', env: { ...DEV_ENV, DB: db } })
  ...
  it('200 returns list ordered by created_at desc', async () => {
    ...
    state.allResults = [
      { submissionId: 's2', userId: 'u_x', problemId: 'p1', status: 'queued', createdAt: 2 },
      { submissionId: 's1', userId: 'u_x', problemId: 'p1', status: 'queued', createdAt: 1 }
    ]
    const sid = await signSession(DEV_ENV.AUTH_SESSION_SECRET, 'alice@studenti.unitn.it')
    const ctx = makeCtx({ url: 'http://x/api/v1/users/me/submissions', headers: { cookie: `sid=${sid}` }, env: { ...DEV_ENV, DB: db } })
    const res = await (mod as any).onRequestGet(ctx)
  ...
  it('500 INTERNAL when DB throws in prod', async () => {
    ...
    ;(db as any).prepare = () => ({ bind: () => ({ all: async () => { throw new Error('boom') } }) })
    const ctx = makeCtx({ url: 'http://x/api/v1/users/me/submissions', headers: { cookie: `sid=${sid}` }, env: { ...PROD_ENV, DB: db } })
    const res = await (mod as any).onRequestGet(ctx)
